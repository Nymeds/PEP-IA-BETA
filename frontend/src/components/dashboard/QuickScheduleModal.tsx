...
import { formatDateHeading, formatDateTimeLabel } from './calendar-utils'
...
export function QuickScheduleModal({ open, date, onClose, onBooked }: Props) {
  ...
  const slotsQuery = useQuery({
    queryKey: ['schedule-slots', date],
    queryFn: () => api.schedule.slots(date || ''),
    enabled: open && Boolean(date),
  })

  const booking = useMutation({
    mutationFn: () =>
      api.schedule.quickBook({
    ...
    onSuccess: async (consultation) => {
      await Promise.all([
        ...
        qc.invalidateQueries({ queryKey: ['schedule-slots', date] }),
      ...
      <div className="card w-full max-w-5xl max-h-[92vh] overflow-hidden flex flex-col">
            ...
            <div className="space-y-2 max-h-72 overflow-y-auto pr-1">
              {patientsQuery.isLoading ? (
                      ...
                      <div className="flex items-center justify-between gap-3">
                        <div>
                          ...
                          <p className="text-xs text-slate-500">
                            {patient.phone || patient.email || (patient.quickCreated ? 'Cadastro rapido' : 'Sem contato informado')}
                          </p>
                        </div>
                        <UserRound className={cn('w-4 h-4', active ? 'text-primary-600' : 'text-slate-300')} />
                      </div>
                    </button>
                  )
                })
                ...
                <div className="rounded-xl border border-dashed border-slate-300 bg-white px-4 py-6 text-center">
                  ...
                  <p className="text-xs text-slate-400 mt-1">Voce ainda pode fazer o cadastro rapido ao confirmar.</p>
                </div>
              )}
            </div>
          </section>
                    ...
                    <div className="space-y-2">
                      {slotsQuery.data.appointments.map((appointment) => (
                        ...
                        >
                          ...
                          <span className="text-xs text-slate-500">{formatDateTimeLabel(appointment.scheduledAt)}</span>
                        </Link>
                      ))}
                    </div>
                  </div>
        ...
        <div className="px-6 py-4 border-t border-slate-100 flex items-center justify-between gap-3">
          <div className="text-sm text-slate-500">
            {selectedSlot ? `Horario selecionado: ${formatDateTimeLabel(selectedSlot)}` : 'Escolha um horario para continuar.'}
          </div>
